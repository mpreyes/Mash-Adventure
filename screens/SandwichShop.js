...
export default class SandwichShopScreen extends React.Component {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    return (
      ...
      <ScrollView style={styles.textStyling}>
        <Text style={styles.questionStyling}>{'\t'}You ask the game creator if there are any good sandwich shops 
        nearby, as you are feeling a bit hungry.</Text>

      <Animatable.View animation="fadeInDown"  direction="alternate" style={{flex: 2}}>
              <Image
            style={{flex:1, height: 350, width: undefined}}
            source={require('../images/sandwiches.png')}
            resizeMethod="resize"
            resizeMode="contain"
          />
      </Animatable.View>
   ...
   {'\t\t'}The game creator regrets to inform you that her favorite sandwich shop is a well known 
      restaurant (which shall not be named) that sells basic sandwiches and rhymes with "BUS WAY". You give the game creator a look of dissapointment,
      since anyone with good taste likes their sandwiches from literally any other place.

     </Text>

    <Animatable.View animation="slideInUp" style={styles.adventureGroupButtonStyling}>
      <View style={styles.adventureSoloButtonStyling}>
  <Button  title="Take the worst ending as punishment for the judgey look"  color="#1698bc"  onPress={() =>
      navigate('Results', {
      profile: profile,
      mash: {
          M: "Mansion",
